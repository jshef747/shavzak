    ...
    <div className="space-y-5">
      ...
      <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4 sm:p-5 shadow-sm">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">{t('qualifiedPositions', lang)}</h3>
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{t('qualifiedDesc', lang)}</p>
          </div>
        </div>
        {state.positions.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-slate-500 text-center py-4">{t('noPositionsDefined', lang)}</p>
        ) : (
      ...
      {dates.length > 0 && (
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4 sm:p-5 shadow-sm">
          <div className="flex items-start gap-3 mb-4">
            <div className="w-9 h-9 rounded-lg bg-red-50 dark:bg-red-900/40 flex items-center justify-center flex-shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">{t('oneTimeConstraints', lang)}</h3>
              <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{t('oneTimeDesc', lang)}</p>
            </div>
          </div>
          <div className="overflow-auto max-h-64 rounded-lg border border-gray-200 dark:border-slate-600">
            <table className="min-w-full text-xs">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                {dates.map((date, i) => (
                  <tr key={date} className={i % 2 === 0 ? 'bg-white dark:bg-slate-800' : 'bg-gray-50/50 dark:bg-slate-700/50'}>
                    <td className="px-3 py-2 whitespace-nowrap">
                      <div dir="ltr" className="flex flex-col items-start leading-tight">
                        <span className="text-xs font-semibold text-gray-700 dark:text-slate-300">{format(parseISO(date), 'EEE')}</span>
      ...
      <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4 sm:p-5 shadow-sm">
        <div className="flex items-start gap-3 mb-5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">{t('repeatingConstraints', lang)}</h3>
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{t('repeatingDesc', lang)}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
            <p className="text-xs font-medium text-gray-600 dark:text-slate-300 mb-2">{t('allowedShifts', lang)}</p>
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                      active
                        ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-300 dark:border-indigo-600 text-indigo-700 dark:text-indigo-300 shadow-sm'
                        : 'bg-white dark:bg-slate-600 border-gray-200 dark:border-slate-500 text-gray-500 dark:text-slate-400 hover:border-indigo-300 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20'
                    }`}
                  >
                    {shift.name}
                  </button>
          ...
          {/* Blocked Shifts */}
          <div className="p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
            <p className="text-xs font-medium text-gray-600 dark:text-slate-300 mb-2">{t('blockedShifts', lang)}</p>
            <div className="flex flex-wrap gap-1.5">
              {state.shifts.map(shift => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                      active
                        ? 'bg-red-50 dark:bg-red-900/30 border-red-300 dark:border-red-700 text-red-700 dark:text-red-400 shadow-sm'
                        : 'bg-white dark:bg-slate-600 border-gray-200 dark:border-slate-500 text-gray-500 dark:text-slate-400 hover:border-red-300 hover:bg-red-50/50 dark:hover:bg-red-900/20'
                    }`}
                  >
                    {shift.name}
                  </button>
          ...
          {/* Allowed Days */}
          <div className="p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
            <p className="text-xs font-medium text-gray-600 dark:text-slate-300 mb-2">{t('allowedDays', lang)}</p>
            <div className="flex flex-wrap gap-1.5">
          ...
          {/* Blocked Days */}
          <div className="p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
            <p className="text-xs font-medium text-gray-600 dark:text-slate-300 mb-2">{t('blockedDays', lang)}</p>
            <div className="flex flex-wrap gap-1.5">
              {dayLabels.map(({ day, label }) => {
          ...
          {/* Numeric Limits */}
          <div className="p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
            <p className="text-xs font-medium text-gray-600 dark:text-slate-300 mb-3">{t('limitsLabel', lang)}</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {([
                ...
                <div key={label} className="flex flex-col gap-1 items-center text-center justify-between">
                  <label className="text-[11px] font-medium text-gray-500 dark:text-slate-400 leading-tight">{label}</label>
                  <input
                    type="number"
                    min={1}
      ...
      <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4 sm:p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">{t('forceMinimumLabel', lang)}</h3>
              <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{t('forceMinimumDesc', lang)}</p>
            </div>
          </div>
          <button
            role="switch"
      ...
      <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-xl p-4 sm:p-5">
        <div className="flex items-start gap-3 mb-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-red-900 dark:text-red-400">{t('dangerZone', lang)}</h3>
