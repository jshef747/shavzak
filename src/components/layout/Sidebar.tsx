...
export function Sidebar({ state, assignments, onEditPerson, onDeletePerson, open, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* Drawer panel */}
      <aside
        className={`no-print fixed inset-y-0 left-0 rtl:left-auto rtl:right-0 z-30 w-64 flex flex-col overflow-y-auto bg-white border-r rtl:border-r-0 rtl:border-l border-slate-200 transition-transform duration-200 ease-out ${
          open ? 'translate-x-0' : '-translate-x-full rtl:translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200 shrink-0">
          <span className="text-sm font-semibold text-slate-700">Roster</span>
          <button
            onClick={onClose}
